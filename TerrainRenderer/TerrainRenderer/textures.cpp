...
bool Textures::init(ID3D11Device *dev, ID3D11DeviceContext *devContext)
{
	...
	m_textures.resize(2);
	hr = D3DX11CreateShaderResourceViewFromFile(dev, L"resources/grass-256x256.png", NULL, NULL, &m_textures[0], NULL);
	hr = D3DX11CreateShaderResourceViewFromFile(dev, L"resources/rocks-256x256.png", NULL, NULL, &m_textures[1], NULL);
	if (FAILED(hr))
	{
		m_textures.resize(0);
		return false;
